(function (tests) {
	...
	QUnit.module( "ERFA", function ()
	{
		QUnit.test( "apco", function( assert )
		{
			...
			{
				var res = eraApco(tests[i][0], tests[i][1], tests[i][2], tests[i][3], tests[i][4], tests[i][5], tests[i][6], tests[i][7], tests[i][8], tests[i][9], tests[i][10], tests[i][11], tests[i][12], tests[i][13], tests[i][14], tests[i][15]);
