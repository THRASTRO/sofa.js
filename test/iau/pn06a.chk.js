(function (tests) {
	...
	{
		QUnit.test( "pn06a", function( assert )
		{
			for (var i = 0; i < tests.length; i += 1)
			{
				var res = iauPn06a(tests[i][0], tests[i][1]);
				...
				assert.close(res[2], tests[i][4], 1e-8, JSON.stringify(tests[i][0]) +', '+ JSON.stringify(tests[i][1]) + ' (epsa)');
