	...
	QUnit.module( "IAU", function ()
	{
		QUnit.test( "apco", function( assert )
		{
			for (var i = 0; i < tests.length; i += 1)
			{
				...
				assert.ASTROM(res, tests[i][16], 1e-8, JSON.stringify(tests[i][0]) +', '+ JSON.stringify(tests[i][1]) +', '+ JSON.stringify(tests[i][2]) +', '+ JSON.stringify(tests[i][3]) +', '+ JSON.stringify(tests[i][4]) +', '+ JSON.stringify(tests[i][5]) +', '+ JSON.stringify(tests[i][6]) +', '+ JSON.stringify(tests[i][7]) +', '+ JSON.stringify(tests[i][8]) +', '+ JSON.stringify(tests[i][9]) +', '+ JSON.stringify(tests[i][10]) +', '+ JSON.stringify(tests[i][11]) +', '+ JSON.stringify(tests[i][12]) +', '+ JSON.stringify(tests[i][13]) +', '+ JSON.stringify(tests[i][14]) +', '+ JSON.stringify(tests[i][15]) + ' (astrom)');
